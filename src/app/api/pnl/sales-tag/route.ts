import { NextRequest, NextResponse } from 'next/server';
import { executeQuery } from '@/lib/snowflake';

export async function GET(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams;
  const brandCode = searchParams.get('brandCode') || 'M';
  const season = searchParams.get('season') || '23S';

  try {
    // 시즌에서 연도와 시즌타입 추출
    const year = parseInt(season.slice(0, 2));
    const seasonType = season.slice(2); // S, F, N
    
    // 시즌별 기간 설정
    let periodEnd: string; // 시즌 종료월
    let periodStart: string; // 시즌 시작월 (과시즌/ACC 기준)
    let currentSeasonStart: string; // 당시즌 의류 시작월
    let pyPeriodEnd: string; // 전년 시즌 종료월
    let pyPeriodStart: string; // 전년 시즌 시작월
    let pyCurrentSeasonStart: string; // 전년 당시즌 의류 시작월
    
    if (seasonType === 'S') {
      // S시즌: 03월~08월
      periodEnd = `20${year}08`;
      periodStart = `20${year}03`;
      currentSeasonStart = `20${year - 1}01`; // 당시즌 의류는 전년 1월부터
      pyPeriodEnd = `20${year - 1}08`;
      pyPeriodStart = `20${year - 1}03`;
      pyCurrentSeasonStart = `20${year - 2}01`;
    } else if (seasonType === 'F') {
      // F시즌: 09월~02월 (다음해)
      periodEnd = `20${year + 1}02`;
      periodStart = `20${year}09`;
      currentSeasonStart = `20${year}03`; // 당시즌 의류는 해당년 3월부터
      pyPeriodEnd = `20${year}02`;
      pyPeriodStart = `20${year - 1}09`;
      pyCurrentSeasonStart = `20${year - 1}03`;
    } else {
      // N시즌: 기본값
      periodEnd = `20${year}12`;
      periodStart = `20${year}01`;
      currentSeasonStart = `20${year - 1}07`;
      pyPeriodEnd = `20${year - 1}12`;
      pyPeriodStart = `20${year - 1}01`;
      pyCurrentSeasonStart = `20${year - 2}07`;
    }

    // 6개월 전 기준 (전시즌 의류 판단용)
    const sixMonthsBefore = seasonType === 'S' 
      ? `20${year}02` 
      : `20${year}08`;
    const pySixMonthsBefore = seasonType === 'S'
      ? `20${year - 1}02`
      : `20${year - 1}08`;

    const query = `
-- 판매TAG 매출
-- cy_item : 당해 아이템 구분 기준
with cy_item as (
    select a.prdt_cd  
            , a.sesn
            , a.prdt_hrrc1_nm
            , a.prdt_hrrc2_nm
            , a.prdt_hrrc3_nm
            , case when ('${periodEnd}' between b.start_yyyymm and b.end_yyyymm) and prdt_hrrc1_nm = '의류' 
                        then '당시즌 의류'
                    when ('${sixMonthsBefore}' between b.start_yyyymm and b.end_yyyymm) and prdt_hrrc1_nm = '의류'
                        then '전시즌 의류'
                    when (b.start_yyyymm > '${periodEnd}') and prdt_hrrc1_nm = '의류' 
                        then '차기시즌 의류'
                    when (b.start_yyyymm < '${sixMonthsBefore}') and prdt_hrrc1_nm = '의류'
                        then '과시즌 의류'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Headwear' 
                        then '모자'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Shoes' 
                        then '신발'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Bag' 
                        then '가방'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Acc_etc' 
                        then '기타ACC'
                    else '기타' end as item_std
    from sap_fnf.mst_prdt a
    left join comm.mst_sesn b
        on a.sesn = b.sesn
    where 1=1
        and brd_cd = '${brandCode}'
)
-- py_item : 전년 아이템 구분 기준
, py_item as (
    select a.prdt_cd  
            , a.sesn
            , a.prdt_hrrc1_nm
            , a.prdt_hrrc2_nm
            , a.prdt_hrrc3_nm
            , case when ('${pyPeriodEnd}' between b.start_yyyymm and b.end_yyyymm) and prdt_hrrc1_nm = '의류' 
                        then '당시즌 의류'
                    when ('${pySixMonthsBefore}' between b.start_yyyymm and b.end_yyyymm) and prdt_hrrc1_nm = '의류'
                        then '전시즌 의류'
                    when (b.start_yyyymm > '${pyPeriodEnd}') and prdt_hrrc1_nm = '의류' 
                        then '차기시즌 의류'
                    when (b.start_yyyymm < '${pySixMonthsBefore}') and prdt_hrrc1_nm = '의류'
                        then '과시즌 의류'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Headwear' 
                        then '모자'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Shoes' 
                        then '신발'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Bag' 
                        then '가방'
                    when prdt_hrrc1_nm='ACC' and prdt_hrrc2_nm='Acc_etc' 
                        then '기타ACC'
                    else '기타' end as item_std
    from sap_fnf.mst_prdt a
    left join comm.mst_sesn b
        on a.sesn = b.sesn
    where 1=1
        and brd_cd = '${brandCode}'
)
-- 당시즌 의류 매출 (전년 1월부터 시즌 종료까지)
, current_season_apparel_cy as (
    select case 
            when b.mgmt_chnl_cd = '4' then '온라인(직)'
            when b.mgmt_chnl_cd = '5' then '온라인(제휴)'
            when b.mgmt_chnl_cd in ('3', '11', 'C3') then '직영(가두)'
            when b.mgmt_chnl_nm like '아울렛%' then '아울렛(직)'
            when b.mgmt_chnl_nm = '백화점' then '백화점'
            when b.mgmt_chnl_nm = '대리점' then '대리점'
            when b.mgmt_chnl_nm = '면세점' then '면세점'
            when a.chnl_cd = '8' then '사입'
            when a.chnl_cd = '99' then '기타'
            else b.mgmt_chnl_nm
         end as chnl_nm
         , sum(a.tag_sale_amt) as sale_tag_amt
    from sap_fnf.dm_pl_shop_prdt_m a
    join sap_fnf.mst_shop b
      on a.brd_cd = b.brd_cd and a.shop_cd = b.sap_shop_cd
    join cy_item c
      on a.prdt_cd = c.prdt_cd
    where a.brd_cd = '${brandCode}'
      and a.corp_cd = '1000'
      and a.chnl_cd not in ('9')
      and a.chnl_cd is not null
      and a.pst_yyyymm between '${currentSeasonStart}' and '${periodEnd}'
      and c.item_std = '당시즌 의류'
    group by 1
)
-- 과시즌 의류 + 전시즌 의류 + ACC 매출 (시즌 기간 내)
, other_sales_cy as (
    select case 
            when b.mgmt_chnl_cd = '4' then '온라인(직)'
            when b.mgmt_chnl_cd = '5' then '온라인(제휴)'
            when b.mgmt_chnl_cd in ('3', '11', 'C3') then '직영(가두)'
            when b.mgmt_chnl_nm like '아울렛%' then '아울렛(직)'
            when b.mgmt_chnl_nm = '백화점' then '백화점'
            when b.mgmt_chnl_nm = '대리점' then '대리점'
            when b.mgmt_chnl_nm = '면세점' then '면세점'
            when a.chnl_cd = '8' then '사입'
            when a.chnl_cd = '99' then '기타'
            else b.mgmt_chnl_nm
         end as chnl_nm
         , sum(a.tag_sale_amt) as sale_tag_amt
    from sap_fnf.dm_pl_shop_prdt_m a
    join sap_fnf.mst_shop b
      on a.brd_cd = b.brd_cd and a.shop_cd = b.sap_shop_cd
    join cy_item c
      on a.prdt_cd = c.prdt_cd
    where a.brd_cd = '${brandCode}'
      and a.corp_cd = '1000'
      and a.chnl_cd not in ('9')
      and a.chnl_cd is not null
      and a.pst_yyyymm between '${periodStart}' and '${periodEnd}'
      and c.item_std not in ('당시즌 의류', '차기시즌 의류')
    group by 1
)
-- 전년 당시즌 의류 매출
, current_season_apparel_py as (
    select case 
            when b.mgmt_chnl_cd = '4' then '온라인(직)'
            when b.mgmt_chnl_cd = '5' then '온라인(제휴)'
            when b.mgmt_chnl_cd in ('3', '11', 'C3') then '직영(가두)'
            when b.mgmt_chnl_nm like '아울렛%' then '아울렛(직)'
            when b.mgmt_chnl_nm = '백화점' then '백화점'
            when b.mgmt_chnl_nm = '대리점' then '대리점'
            when b.mgmt_chnl_nm = '면세점' then '면세점'
            when a.chnl_cd = '8' then '사입'
            when a.chnl_cd = '99' then '기타'
            else b.mgmt_chnl_nm
         end as chnl_nm
         , sum(a.tag_sale_amt) as sale_tag_amt
    from sap_fnf.dm_pl_shop_prdt_m a
    join sap_fnf.mst_shop b
      on a.brd_cd = b.brd_cd and a.shop_cd = b.sap_shop_cd
    join py_item c
      on a.prdt_cd = c.prdt_cd
    where a.brd_cd = '${brandCode}'
      and a.corp_cd = '1000'
      and a.chnl_cd not in ('9')
      and a.chnl_cd is not null
      and a.pst_yyyymm between '${pyCurrentSeasonStart}' and '${pyPeriodEnd}'
      and c.item_std = '당시즌 의류'
    group by 1
)
-- 전년 과시즌 의류 + 전시즌 의류 + ACC 매출
, other_sales_py as (
    select case 
            when b.mgmt_chnl_cd = '4' then '온라인(직)'
            when b.mgmt_chnl_cd = '5' then '온라인(제휴)'
            when b.mgmt_chnl_cd in ('3', '11', 'C3') then '직영(가두)'
            when b.mgmt_chnl_nm like '아울렛%' then '아울렛(직)'
            when b.mgmt_chnl_nm = '백화점' then '백화점'
            when b.mgmt_chnl_nm = '대리점' then '대리점'
            when b.mgmt_chnl_nm = '면세점' then '면세점'
            when a.chnl_cd = '8' then '사입'
            when a.chnl_cd = '99' then '기타'
            else b.mgmt_chnl_nm
         end as chnl_nm
         , sum(a.tag_sale_amt) as sale_tag_amt
    from sap_fnf.dm_pl_shop_prdt_m a
    join sap_fnf.mst_shop b
      on a.brd_cd = b.brd_cd and a.shop_cd = b.sap_shop_cd
    join py_item c
      on a.prdt_cd = c.prdt_cd
    where a.brd_cd = '${brandCode}'
      and a.corp_cd = '1000'
      and a.chnl_cd not in ('9')
      and a.chnl_cd is not null
      and a.pst_yyyymm between '${pyPeriodStart}' and '${pyPeriodEnd}'
      and c.item_std not in ('당시즌 의류', '차기시즌 의류')
    group by 1
)
-- 당해 합산
, cy_combined as (
    select chnl_nm, sale_tag_amt from current_season_apparel_cy
    union all
    select chnl_nm, sale_tag_amt from other_sales_cy
)
-- 전년 합산
, py_combined as (
    select chnl_nm, sale_tag_amt from current_season_apparel_py
    union all
    select chnl_nm, sale_tag_amt from other_sales_py
)
-- 채널별 합계
, channel_summary as (
    select 
        coalesce(cy.chnl_nm, py.chnl_nm) as chnl_nm,
        coalesce(cy.sale_tag_amt, 0) as sale_tag_amt_cy,
        coalesce(py.sale_tag_amt, 0) as sale_tag_amt_py
    from (select chnl_nm, sum(sale_tag_amt) as sale_tag_amt from cy_combined group by 1) cy
    full outer join (select chnl_nm, sum(sale_tag_amt) as sale_tag_amt from py_combined group by 1) py
    on cy.chnl_nm = py.chnl_nm
)
-- 전체 합계
, total_summary as (
    select '전체' as chnl_nm
         , sum(sale_tag_amt_cy) as sale_tag_amt_cy
         , sum(sale_tag_amt_py) as sale_tag_amt_py
    from channel_summary
)
select chnl_nm as CHNL_NM
    , round(sale_tag_amt_cy / 1000000) as SALE_TAG_AMT
    , case when sale_tag_amt_py = 0 then null 
           else round(sale_tag_amt_cy / sale_tag_amt_py * 100) 
      end as YOY
from channel_summary
union all
select chnl_nm as CHNL_NM
    , round(sale_tag_amt_cy / 1000000) as SALE_TAG_AMT
    , case when sale_tag_amt_py = 0 then null 
           else round(sale_tag_amt_cy / sale_tag_amt_py * 100) 
      end as YOY
from total_summary
order by 
    case CHNL_NM
        when '전체' then 0
        when '백화점' then 1
        when '대리점' then 2
        when '면세점' then 3
        when '직영(가두)' then 4
        when '온라인(직)' then 5
        when '온라인(제휴)' then 6
        when '아울렛(직)' then 7
        when '사입' then 8
        when '기타' then 9
        else 10
    end
`;

    const rows = await executeQuery(query);

    // 총합 계산 (전체 행 제외)
    const channelData = rows.filter((row: any) => row.CHNL_NM !== '전체');
    const totalRow = rows.find((row: any) => row.CHNL_NM === '전체');

    return NextResponse.json({
      success: true,
      data: channelData,
      total: totalRow?.SALE_TAG_AMT || 0,
      totalYOY: totalRow?.YOY || null,
      params: {
        brandCode,
        season,
        periodStart,
        periodEnd,
        currentSeasonStart,
        pyPeriodStart,
        pyPeriodEnd,
        pyCurrentSeasonStart
      }
    });
  } catch (error) {
    console.error('Sales Tag API Error:', error);
    return NextResponse.json(
      { success: false, error: String(error) },
      { status: 500 }
    );
  }
}
