...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const sixMonthsBefore = seasonType === 'S' ? `20${year}02` : `20${year}08`;
    const pySixMonthsBefore = seasonType === 'S' ? `20${year - 1}02` : `20${year - 1}08`;
...
`;
    ...
    // CSV 직렬화
    const headers = ['CHNL_NM', 'ACT_SALE_AMT', 'YOY', 'CHNL_CD', 'RETAIL_ACT_SALE_AMT'];
    const csv = [
      headers.join(','),
      ...rows.map((r: any) =>
        [
    ...
    const filename = `actual-sales-${brandCode}-${season}.csv`;
    ...
    console.error('Actual Sales Export Error:', error);
